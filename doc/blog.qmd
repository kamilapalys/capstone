...
def naive_bayes_cf(df, value_type):
    ...
    average_accuracies.append((cum_acc/len(accuracy_list), value_type))

    # uncomment lines below if interested in displaying min/max accuracy scores for the models trained within the function
...
```{python}
for entry in average_accuracies:
  print(f'Average accuracy for Naive Bayes models that were trained on {entry[1]}:', round(entry[0], 2))
```
...
def random_forest_cf(df, value_type, n, crit):
    ...
    # uncomment lines below if interested in displaying min/max accuracies of models trained within this function.
...
```{python}
for entry in average_accuracies:
  print(f'Average accuracy for Random Forest models trained on {entry[1]}, using {entry[2]} n_estimators, and {entry[3]} criterion:', round(entry[0], 2))

# if interested in displaying best parameters for random forest models, uncomment line below
# best_rf_params
```
...
X_train_df = binary_df[(binary_df['article_topic'] != 'The Deadliest Attack by Hamas') & 
        (binary_df['article_topic'] != 'Pentagon Documents Leak')].iloc[:, :-2]
X_test_df = binary_df[(binary_df['article_topic'] == 'The Deadliest Attack by Hamas') | 
        (binary_df['article_topic'] == 'Pentagon Documents Leak')].iloc[:, :-2]
y_train_df = binary_df[(binary_df['article_topic'] != 'The Deadliest Attack by Hamas') & 
        (binary_df['article_topic'] != 'Pentagon Documents Leak')]['article_source']
y_test_df = binary_df[(binary_df['article_topic'] == 'The Deadliest Attack by Hamas') | 
        (binary_df['article_topic'] == 'Pentagon Documents Leak')]['article_source']
X_train = X_train_df.to_numpy()
X_test = X_test_df.to_numpy()
y_train = y_train_df.to_numpy()
